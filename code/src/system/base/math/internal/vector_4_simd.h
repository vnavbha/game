...
/// @file internal/vector_4_simd.h MathFu Vector<T, 4> Specialization
/// @brief 4-dimensional specialization of Vector for SIMD optimized
/// builds.
/// @see Vector
...
class Vector<float, 4> {
 public:
  ...
  static inline Vector<float, 4> RandomInRange(const Vector<float, 4>& min,
    ...
    return Vector<float, 4>(RandomInRange<float>(min[0], max[0]),
                            RandomInRange<float>(min[1], max[1]),
                            RandomInRange<float>(min[2], max[2]),
                            RandomInRange<float>(min[3], max[3]));
  }
