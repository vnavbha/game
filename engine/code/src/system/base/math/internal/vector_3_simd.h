...
/// @file internal/vector_3_simd.h MathFu Vector<T, 3> Specialization
/// @brief 3-dimensional specialization of Vector for SIMD optimized
/// builds.
/// @see Vector
...
class Vector<float, 3> {
 public:
  ...
  static inline Vector<float, 3> RandomInRange(const Vector<float, 3>& min,
    ...
    return Vector<float, 3>(RandomInRange<float>(min[0], max[0]),
