...
/// @file internal/vector_2_simd.h MathFu Vector<T, 2> Specialization
/// @brief 2-dimensional specialization of Vector for SIMD optimized
/// builds.
/// @see Vector
...
class Vector<float, 2> {
 public:
  ...
  static inline Vector<float, 2> RandomInRange(const Vector<float, 2>& min,
    ...
    return Vector<float, 2>(RandomInRange<float>(min[0], max[0]),
