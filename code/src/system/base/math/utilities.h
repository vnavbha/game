...
/// For example:
/// <blockquote><code><pre>
/// std::vector<vec4, simd_allocator<vec4>> myvector;
/// </pre></code></blockquote>
///
...
#define MATHFU_DEFINE_GLOBAL_SIMD_AWARE_NEW_DELETE                           \
  void *operator new(std::size_t n) { return AllocateAligned(n); }   \
  void *operator new[](std::size_t n) { return AllocateAligned(n); } \
  void operator delete(void *p) noexcept { FreeAligned(p); }         \
  void operator delete[](void *p) noexcept { FreeAligned(p); }
...
#define MATHFU_DEFINE_CLASS_SIMD_AWARE_NEW_DELETE                       \
  ...
  static void *operator new[](std::size_t /*n*/, void *p) { return p; } \
  static void operator delete(void *p) { FreeAligned(p); }      \
  static void operator delete[](void *p) { FreeAligned(p); }    \
